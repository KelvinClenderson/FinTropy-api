...
class UpdateTransactionService {
    ...
    async execute({ id, workspaceId, ...data }) {
        ...
        const currentIsInstallment = (transaction.totalInstallments && transaction.totalInstallments > 1) || false;
        ...
        console.log('MODO DE EDIÇÃO:', isEditingInstallmentLogic ? '>>> DELETE & RECREATE' : '>>> SIMPLE UPDATE');
        // ==================================================================================
        ...
        if (isEditingInstallmentLogic) {
            ...
            if (targetInstallmentNumber > 1) {
                basePurchaseDate = (0, date_fns_1.subMonths)(targetDate, targetInstallmentNumber - 1);
            }
            ...
            const createService = new create_transaction_service_1.CreateTransactionService(this.transactionsRepository);
        ...
        if (data.amount) {
            dataToUpdate.amount = new client_1.Prisma.Decimal(data.amount);
            dataToUpdate.totalAmount = new client_1.Prisma.Decimal(data.amount);
        }
        if (data.type)
            dataToUpdate.type = data.type;
        if (data.paymentMethod)
            dataToUpdate.paymentMethod = data.paymentMethod;
        if (data.observation !== undefined)
            dataToUpdate.observation = data.observation;
        if (data.payee !== undefined)
            dataToUpdate.payee = data.payee;
        if (data.categoryId)
