...
const prisma_1 = require("../../lib/prisma"); // Importamos o prisma para usar transaction
class DeleteCategoryService {
    constructor(categoriesRepository) {
        this.categoriesRepository = categoriesRepository;
    }
    async execute({ id, workspaceId, substituteCategoryId }) {
        // 1. Buscar a Categoria a ser Deletada
        const categoryToDelete = await this.categoriesRepository.findById(id);
        if (!categoryToDelete)
            throw new Error('Categoria a ser excluída não encontrada.');
        if (categoryToDelete.workspaceId !== workspaceId)
        ...
        if (!substituteCategory)
            throw new Error('Categoria substituta não encontrada.');
